...
1. **Audio generation** (`scripts/generate_audio.py`) — Kokoro TTS synthesizes narration clips for each scene. Outputs timestamped `.wav` files and a `durations.json` used by the recorder for audio sync.

2. **Recording** (`src/record.ts`) — Starts a temporary Nebi server (SQLite, in-memory queue), seeds workspaces/users/registries via the API, then drives the UI through 16 scenes with Playwright. Each scene records a timestamp so audio can be aligned later. The `nebi` binary is resolved from `PATH`, `./bin/nebi`, or downloaded from the latest GitHub release.

3. **Conversion** (`scripts/convert.sh`) — Takes the raw `.webm` recording, mixes in the narration clips at the correct timestamps using FFmpeg's `adelay` + `amix` filters, and produces the final `.mp4`. Also generates a 5x speed `.gif` for embedding.
...
| File | Purpose |
|------|---------|
| `src/record.ts` | Playwright recording script (server lifecycle, data seeding, 16 scene walkthrough) |
| `src/annotations.ts` | Cursor animation, tooltip overlays, and final screen helpers |
| `scripts/generate_audio.py` | Kokoro TTS narration generator |
| `scripts/convert.sh` | FFmpeg audio merge + MP4/GIF conversion |
| `assets/` | Static assets (avatar SVG, logo PNG) |
